...
class EvalInst(object):
    ...
    def evaluate_inst(self, dataset_seg, dataset_inst, topk):
        ...
        eval_tool_all = EvalSceneGraphBatch(node_cls_names, edge_cls_names,
                                            multi_rel_prediction=self.cfg.model.multi_rel, k=100, save_prediction=True,
    ...
    def evaluate_inst_recall(self, dataset_seg, dataset_inst, topk):
        is_eval_image = self.cfg.model.method in ['imp']
        ignore_missing = self.cfg.eval.ignore_missing
        ...
        for index in tqdm(range(len(dataset_inst))):
            try:
                ...
                with torch.no_grad():
                    ...
                    '''merge prediction from seg to instance (in case of "same part")'''
                    # use list may have multiple predictions on the same object instance
                    seg_oid2idx = defaultdict(list) 
                    for idx in range(len(seg_oids)):
                        seg_oid2idx[seg_oids[idx]].append(idx)
                    '''merge nodes'''
                        ...
                        if not ignore_missing:
                            ...
                            if oid in seg_oid2idx:
                                ...
                                merged_node_cls[idx,
                                                inst_valid_node_cls_indices] = node_cls_pred[seg_valid_node_cls_indices]
                                # print(predictions)
                                # print(merged_node_cls)
                                # print(merged_node_cls_gt)
                                # print(merged_node_cls.shape)
                                # print(merged_node_cls_gt.shape)
                                # print(node_cls_pred.shape)
                                # print(node_cls.shape)
                                # return
                            else:
                                assert noneidx_node_cls is not None
                ...
                data_inst['node'].clsIdx = torch.from_numpy(
                    np.array([k for k in merged_idx2oid.values()]))
                # Top K predictions
                _, node_topk = torch.topk(data_inst['node'].pd, 8, dim=1)
                _, edge_topk = torch.topk(data_inst['node', 'to', 'node'].pd, 2, dim=1)
                # print(data_inst['node'].pd)
                # print(torch.max(data_inst['node'].pd, dim=1))
                # print(torch.argmax(data_inst['node'].pd, dim=1))
                # # print(data_inst['node'].pd[:, torch.argmax(data_inst['node'].pd, dim=1)])
                
                # print(data_inst['node'].y )
                # print(node_topk)
                # # print(data_inst['node', 'to', 'node'].pd)
                # print(torch.argmax(data_inst['node', 'to', 'node'].pd, dim=1))
                # # print(data_inst['node', 'to', 'node'].pd[torch.argmax(data_inst['node', 'to', 'node'].pd, dim=1)])
                # print(data_inst['node', 'to', 'node'].y)
                # print(edge_topk)
                # # print(data_inst['node'].clsIdx)
                # print(data_inst['node', 'to', 'node'].edge_index)
                # return

                
                # Node-level recall
                for i in range(data_inst['node'].y.size(0)):
                    if data_inst['node'].y[i].item() in node_topk[i]:
                        node_correct += 1
                    node_total += 1

                # Edge-level recall
                for i in range(data_inst['node', 'to', 'node'].y.size(0)):
                    if data_inst['node', 'to', 'node'].y[i].item() in edge_topk[i]:
                        edge_correct += 1
                    edge_total += 1
                    
                
                # Triplet-level recall
                # print(data_inst['node', 'to', 'node'].edge_index)
                # print(data_inst['node'].pd)
                # print(data_inst['node'].clsIdx)
                # print(data_inst['node'].y )
                # return
                for i in range(data_inst['node', 'to', 'node'].edge_index.size(1)):
                    # Get the nodes connected by the edge
                    node1_idx = data_inst['node', 'to', 'node'].edge_index[0, i]
                    node2_idx = data_inst['node', 'to', 'node'].edge_index[1, i]

                    # True labels for the triplet
                    true_node1 = data_inst['node'].y[node1_idx].item()
                    true_edge = data_inst['node', 'to', 'node'].y[i].item()
                    true_node2 = data_inst['node'].y[node2_idx].item()

                    # Predicted top K for the triplet
                    node1_topk = node_topk[node1_idx]
                    edge_topk_values = edge_topk[i]
                    node2_topk = node_topk[node2_idx]
                    # print(data_inst['node', 'to', 'node'].y)
                    # print(node1_topk, true_node1, node1_idx)
                    # print(node2_topk, true_node2, node2_idx)
                    # print(edge_topk_values, true_edge)
                    # # Check if the true triplet is within the top K predictions
                    # print(true_node1 in node1_topk)
                    # print(true_edge in edge_topk_values)
                    # print(true_node2 in node2_topk)
                    if (true_node1 in node1_topk) and (true_edge in edge_topk_values) and (true_node2 in node2_topk):
                        triplet_correct += 1
                    triplet_total += 1
                    # return
                ...
                pass
        # Calculate recall
        node_recall = node_correct / node_total if node_total > 0 else 0
        edge_recall = edge_correct / edge_total if edge_total > 0 else 0
        triplet_recall = triplet_correct / triplet_total if triplet_total > 0 else 0
        print(node_recall, edge_recall, triplet_recall)
        return
